...
@Service
@Slf4j
public class MoneyBundleCacheServiceImpl extends CacheableDataCacheServiceImplBase<Integer, MoneyBundle> implements MoneyBundleCacheService {
    ...
    @Override
    protected Optional<MoneyBundle> loadReferenceDataById(Integer id) {
        log.debug("Load money bundle with ID {}", id);
        return moneyBundleRepository.findById(id);
    }

    @Override
    protected CacheableDataPage<MoneyBundle> loadReferenceDataPage(int page, int size) {
