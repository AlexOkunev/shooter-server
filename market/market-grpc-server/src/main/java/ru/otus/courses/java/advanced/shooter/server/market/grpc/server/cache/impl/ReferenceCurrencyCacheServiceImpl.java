...
@Service
@Slf4j
public class ReferenceCurrencyCacheServiceImpl extends CacheableDataCacheServiceImplBase<Integer, ReferenceCurrency> implements ReferenceCurrencyCacheService {
    ...
    @Override
    protected Optional<ReferenceCurrency> loadReferenceDataById(Integer id) {
        log.debug("Load currency with ID {}", id);
        return referenceCurrencyRepository.findById(id);
    }

    @Override
    protected CacheableDataPage<ReferenceCurrency> loadReferenceDataPage(int page, int size) {
