...
import ru.otus.courses.java.advanced.shooter.server.gateway.player.service.cache.*;
...
public class ScheduledTaskConfig {
    ...
    @ConditionalOnProperty(
            name = {
                    "caches.enabled",
                    "caches.player.reset-by-schedule-enabled"
            },
            havingValue = "true"
    )
    public ScheduledFuture<?> resetPlayerCacheTask(TaskScheduler taskScheduler,
                                                   PlayerCacheService playerCacheService,
                                                   @Value("${caches.player.reset-interval-ms:3600000}") long interval) {
        return taskScheduler.schedule(
                () -> {
                    log.info("Resetting player cache");
                    playerCacheService.reset();
                    log.info("Player cache has been reset. Current size: {}", playerCacheService.count());
                },
                new PeriodicTrigger(Duration.ofMillis(interval)));
