...
import ru.otus.courses.java.advanced.shooter.server.gateway.admin.properties.InventoryGrpcProperties;
...
public class InventoryGrpcClientConfig {
    ...
    @Bean
    @Scope(BeanDefinition.SCOPE_PROTOTYPE)
    public InitialPlayerInventoryServiceAPIGrpc.InitialPlayerInventoryServiceAPIBlockingStub initialInventoryServiceAPIBlockingStub(
            @Qualifier(INVENTORY_CHANNEL_NAME) ManagedChannel managedChannel
    ) {
        return InitialPlayerInventoryServiceAPIGrpc.newBlockingStub(managedChannel)
                .withDeadline(Deadline.after(inventoryGrpcProperties.getDeadlineMs(), TimeUnit.MILLISECONDS));
    }
}
