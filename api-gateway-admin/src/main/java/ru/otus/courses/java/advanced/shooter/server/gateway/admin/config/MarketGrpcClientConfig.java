...
import ru.otus.courses.java.advanced.shooter.server.gateway.admin.properties.MarketGrpcProperties;
...
public class MarketGrpcClientConfig {
    ...
    @Bean
    @Scope(BeanDefinition.SCOPE_PROTOTYPE)
    public InitialPlayerAccountServiceAPIGrpc.InitialPlayerAccountServiceAPIBlockingStub initialAccountServiceAPIBlockingStub(
            @Qualifier(MARKET_CHANNEL_NAME) ManagedChannel managedChannel
    ) {
        return InitialPlayerAccountServiceAPIGrpc.newBlockingStub(managedChannel)
                .withDeadline(Deadline.after(marketGrpcProperties.getDeadlineMs(), TimeUnit.MILLISECONDS));
    }
}
